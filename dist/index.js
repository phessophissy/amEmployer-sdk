...
var TaskManagerContract = class {
  constructor(address, providerOrSigner) {
    ...
    this.contract = new ethers.ethers.Contract(address, TASK_MANAGER_ABI, providerOrSigner);
  }
  get address() {
  ...
  async getWorkerStats(walletAddress) {
    ...
    return {
      ...
      earnings: ethers.ethers.formatEther(earnings),
      isRegistered
    };
  }
  /**
  ...
  async getPlatformStats() {
    ...
    return {
      ...
      completedTasks: Number(completedTasks),
      paidOut: ethers.ethers.formatEther(paidOut)
    };
  }
  async taskCounter() {
  ...
  async createTask(rewardWei, metadataHash, deadlineDuration) {
    ...
    const iface = new ethers.ethers.Interface(Array.from(TASK_MANAGER_ABI));
   ...
   */
  async approveToken(amount = ethers.ethers.MaxUint256) {
    const signer = this.assertSigner();
    const tokenAddress = await this.paymentToken();
    const tokenContract = new ethers.ethers.Contract(tokenAddress, ERC20_ABI, signer);
...
var WorkerRegistryContract = class {
  constructor(address, providerOrSigner) {
    this.providerOrSigner = providerOrSigner;
    this.contract = new ethers.ethers.Contract(address, WORKER_REGISTRY_ABI, providerOrSigner);
  }
  get address() {
    return this.contract.target;
  }
  ...
  async mintWorkerNFT(workerAddress, personaName, workerType = WorkerTypeOnChain.HUMAN) {
    ...
    const receipt = await tx.wait();
    const iface = new ethers.ethers.Interface(Array.from(WORKER_REGISTRY_ABI));
    for (const log of receipt.logs) {
      try {
        const parsed = iface.parseLog(log);
...
var AmEmployer = class {
  constructor(config = {}) {
    ...
    const providerOrSigner = config.signer ? config.signer : new ethers.ethers.JsonRpcProvider(config.rpcUrl ?? RPC_URLS[CHAIN_IDS.CELO_MAINNET]);
...
exports.AmEmployer = AmEmployer;
exports.AmEmployerApiError = AmEmployerApiError;
exports.AmEmployerClient = AmEmployerClient;
exports.CHAIN_IDS = CHAIN_IDS;
exports.CONTRACT_ADDRESSES = CONTRACT_ADDRESSES;
exports.CUSD_ADDRESSES = CUSD_ADDRESSES;
exports.CUSD_DECIMALS = CUSD_DECIMALS;
exports.DEFAULT_API_URL = DEFAULT_API_URL;
exports.DEFAULT_PLATFORM_FEE_PERCENT = DEFAULT_PLATFORM_FEE_PERCENT;
...
exports.TASK_MANAGER_ABI = TASK_MANAGER_ABI;
exports.TaskManagerContract = TaskManagerContract;
exports.VALIDATION_PASS_SCORE = VALIDATION_PASS_SCORE;
exports.WORKER_REGISTRY_ABI = WORKER_REGISTRY_ABI;
exports.WorkerRegistryContract = WorkerRegistryContract;
exports.WorkerTypeOnChain = WorkerTypeOnChain;
